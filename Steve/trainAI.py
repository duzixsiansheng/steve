    ...
    def FileReName(self):
        ...
        for type in testType:
            ...
            for subclass in subfolder:
                file_counter += 1
                os.rename(FilePath + type + '/' + subclass,
                          FilePath + type + '/' + str(type_counter) + '_' + type + '_' + str(file_counter) + '.jpg')
            type_counter += 1
...
def Main():
    ...
    Train = Training(batch_size= 128, number_batch = 30, catagories = 2, train_folder = 'train/')
    Train.train()
